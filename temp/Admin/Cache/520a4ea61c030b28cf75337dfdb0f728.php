...
<html>
<head>
<title>订单列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
  ...
  <div class="center_r">
    <div class="center">
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        <label>关键字</label>
        <input type="text" class="input1" name="keys" value="<?php echo ($keys); ?>" placeholder="请填写订单号" />
        <label>商城</label>
        ...
        <select name="status" id="status" class="choose">
            <option value="" >请选择</option>
            <?php if(is_array($statuslist)): $k = 0; $__LIST__ = $statuslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><option value="<?php echo ($k); ?>" ><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
        
        
        
        <?php if($is_qx==0): if(!empty($p_id)||!empty($c_id)||!empty($q_id)||!empty($keys)||!empty($istj1)||!empty($uid)): ?><a href="<?php echo U('Shop/taocan');?>">取消查询</a><?php endif; endif; ?>
        <?php if($is_qx==1): if(!empty($q_id)||!empty($keys)||!empty($uid)): ?><a href="<?php echo U('Shop/taocan');?>">取消查询</a><?php endif; endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["ordersn"]); ?></td>
          <td width="135"><?php echo ($vo["price"]); ?></td>
          <td width="80"><?php echo ($vo["psname"]); ?></td>
          <td width="80"><?php echo ($vo["psprice"]); ?></td>
          <td width="80"><?php echo ($vo["zfname"]); ?></td>
          <td width="80"><?php echo ($vo["status_f"]); ?></td>
          
          <td width="135">
        <?php if($vo[status]==4): ?><a href="<?php echo U('Shop/fh',array('id'=>$vo[id]));?>">发货</a><?php endif; ?>
        <?php if($vo[status]==2): ?><a name="<?php echo ($vo["ordersn"]); ?>" link="<?php echo U('Shop/qr_order',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="qr">确认收货</a><?php endif; ?>
        <?php if($vo[status]==3): ?><a href="<?php echo U('Shop/th',array('id'=>$vo[id]));?>">退货处理</a><?php endif; ?>
             <a name="<?php echo ($vo["ordersn"]); ?>" href="<?php echo U('Shop/ins_order',array('id'=>$vo[id]));?>" style="cursor:pointer" >查看详情</a> 
              
              <a name="<?php echo ($vo["ordersn"]); ?>" link="<?php echo U('Shop/del_order',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
...
function gbname(tname,tid){
    $.ajax({
        ...
        cache:false,
        datatype:"JSON",
        url:"<?php echo U('Shop/ajax_updatetcname');?>",
        type:"POST",
        success:function(msg){
            if(msg==1){
                $("#t_"+tid).html(tname);
