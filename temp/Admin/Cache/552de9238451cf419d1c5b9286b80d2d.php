...
<html>
<head>
<title>合影列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Heying/add_hy');?>";
	})
})
function close_win(){
	$(".theme-popover").html("");
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：合影列表</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if($is_qx==0): if(!empty($p_id)||!empty($c_id)||!empty($keys)||!empty($uid)): ?><a href="<?php echo U('Sheji/index');?>">取消查询</a><?php endif; endif; ?>
        <?php if($is_qx==1): if(!empty($keys)||!empty($uid)): ?><a href="<?php echo U('Sheji/index');?>">取消查询</a><?php endif; endif; ?>
        
        <input name="sub" type="submit"  value="查询" class="sub1" />
        
        
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加合影</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php if(!empty($vo[img])): ?><img src="<?php echo ($systemConfig["SITE_INFO"]["url"]); echo ($vo["img"]); ?>" width="100" /><?php endif; ?></td>
        
          <td width="135"><?php echo ($vo["is_tj_f"]); ?></td>
          <td width="135"><?php echo ($vo["status_f"]); ?></td>
          
          <td width="135">
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Heying/xgtatus',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
              <?php if($vo[is_tj]==0): ?><a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Heying/xgis_tj',array('id'=>$vo[id],'is_tj'=>$vo[is_tj]));?>" style="cursor:pointer" class="sgdtstatus">将其推荐到荣誉俱乐部</a><br/><?php endif; ?>
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Heying/edit_hy',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Heying/del_hy',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
