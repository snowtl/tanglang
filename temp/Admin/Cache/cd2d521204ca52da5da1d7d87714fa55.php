    ...
    <div class="center">
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <select id="stid" name="stid">
         ...
         <?php if($is_qx==1): if($keys!=''||$q_id!=''||$stid!=''): ?><a href="<?php echo U('Shiti/shenqing');?>">取消查询</a><?php endif; ?>
        <?php else: ?>
        <?php if($keys!=''||$p_id!=''||$c_id!=''||$q_id!=''||$stid!=''): ?><a href="<?php echo U('Shiti/shenqing');?>">取消查询</a><?php endif; endif; ?>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><a href="<?php echo ($vo["link"]); ?>" target="_blank"><?php echo ($vo["title"]); ?></a></td>
          <td width="135">
              <a name="<?php echo ($vo["id"]); ?>" link="<?php echo U('Shiti/del_sq',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
