...
<html>
<head>
<title>优惠券使用列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
        <div class="center_r_t">您的位置：<a href="<?php echo U('Shop/list_yhq');?>" class="a1" >优惠券列表</a> >>优惠券使用列表</div>
      <div class="center_r_z">
      <form method="get" name="search_form" id="search_form" action=""  >
          <label>优惠券使用状况</label>
          <select class="choose" id="issy" name="issy">
              <option value="">请选择</option>
              <option value="1" <?php if($issy==1): ?>selected<?php endif; ?>>已使用</option>
              <option value="2" <?php if($issy==2): ?>selected<?php endif; ?>>未使用</option>
          </select>
            <?php if($issy==2||$issy==1): ?><a href="<?php echo U('Shop/list_xx_yhq',array('id'=>$id));?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          <td width="135"><?php echo ($vo["yid"]); ?></td>
          <td width="135"><?php echo ($vo["yhqpwd"]); ?></td>
          <td width="135"><?php echo ($vo["issy_f"]); ?></td>
          
          <td width="135">
        <?php if($vo[issy]==0): ?><a name="<?php echo ($vo["id"]); ?>" href="<?php echo U('Shop/edit_status_xxyhq',array('yid'=>$vo[yid]));?>" style="cursor:pointer" >设为已使用</a><br/>
              
              <a name="<?php echo ($vo["id"]); ?>" link="<?php echo U('Shop/del_xx_yhq',array('id'=>$vo[yid]));?>" style="cursor:pointer" class="del">删除</a>
              <?php else: ?>
              <a href="<?php echo U('Shop/yhqjl',array('yid'=>$vo[yid],'id'=>$id));?>">查看使用会员</a><?php endif; ?>
          </td>
