...
<html>
<head>
<title>小区列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Sqtz/addxq');?>";
	})
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：小区列表</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if($is_qx==1): if($keys!=''||$q_id!=''): ?><a href="<?php echo U('Sqtz/listxq');?>">取消查询</a><?php endif; ?>
            <?php else: ?>
        <?php if($keys!=''||$p_id!=''||$c_id!=''||$q_id!=''): ?><a href="<?php echo U('Sqtz/listxq');?>">取消查询</a><?php endif; endif; ?>
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加小区</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135">
            <a name="<?php echo ($vo["id"]); ?>" link="<?php echo U('Sqtz/delxq',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
            <a aid="<?php echo ($vo["id"]); ?>" href="<?php echo U('Sqtz/editxq',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
          </td>
