...
<html>
<head>
<title>回复评论-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
        <div class="center_r_t">您的位置：<a href="<?php echo U('Shop/comments');?>" class="a1" >评论列表</a> >>回复回复评论</div>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <tr class="center_t_d">
          ...
          <td><?php echo ($info["pzfs"]); ?></td>
        </tr>
        <tr class="center_t_d">
          <td width="135">性价比：</td>
          <td><?php echo ($info["xjbfs"]); ?></td>
        </tr>
        <tr class="center_t_d">
          <td width="135">服务分数：</td>
          <td><?php echo ($info["fwfs"]); ?></td>
        </tr>
        <tr class="center_t_d">
          <td width="135">评论标题：</td>
        ...
        <tr class="center_t_d">
          <td width="135">照片：</td>
          <td>
        <?php if($imglist): if(is_array($imglist)): $i = 0; $__LIST__ = $imglist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a target="_blank" href="<?php echo ($systemConfig["SITE_INFO"]["url"]); echo ($vo); ?>"><img width='100' src='__ROOT__<?php echo ($vo); ?>'/></a><br/><?php endforeach; endif; else: echo "" ;endif; endif; ?>
          </td>
        </tr>
        <tr class="center_t_d">
          <td width="135">回复：</td>
