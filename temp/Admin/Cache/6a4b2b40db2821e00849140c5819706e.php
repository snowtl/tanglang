...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      ...
      <div class="center1">
        <div class="on_line">
            <label><a href="<?php echo U('Member/dianpu');?>">材料商</a></label>
          <label><a href="<?php echo U('Member/dianpu',array('is_sq'=>1,'status'=>0));?>">未通过申请 列表</a></label>
          <label class="over">已通过申请 列表</label>
        </div>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
          ...
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
              ...
              <td width="135"><?php echo ($vo["addtime"]); ?></td>
              <td width="100"><?php echo ($vo["status_f"]); ?></td>
              <td width="100"><?php echo ($vo["sex_f"]); ?></td>
              <td width="135">
              <a aid="<?php echo ($vo["u_id"]); ?>" class='chakan' style="cursor:pointer;">查看</a>
              <a name="<?php echo ($vo["u_id"]); ?>" link="<?php echo U('Member/del_shenqing',array('aid'=>$vo[u_id]));?>" style="cursor:pointer" class="del">删除</a>
              <a name="<?php echo ($vo["u_id"]); ?>" link="<?php echo U('Member/qx_shenqing',array('aid'=>$vo[u_id],'type'=>3));?>" style="cursor:pointer" class="shenhe">未通过</a>
...
//删除
    $(".del").click(function(){
	 ...
	 $(".shenhe").click(function(){
        var n=$(".shenhe").attr("name");
        if(confirm("您确定要取消该项申请["+n+"]吗？")){
            window.location.href=$(".shenhe").attr("link");
        }
    });
	//获取城市
...
$(".chakan").click(function(){
	...
	$.ajax({
		...
		type:"POST",
		dataType:"JSON",
		url:"<?php echo U('Member/getshenqing');?>",
		cache:false,
		success: function(msg){
			msg=eval("("+msg+")");
