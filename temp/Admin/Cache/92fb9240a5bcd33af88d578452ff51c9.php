...
<html>
<head>
<title>广告列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Ad/addad');?>";
	})
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：广告列表</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
       ...
       <?php if($is_qx==0): ?><label>省</label>
        ...
        <?php if($keys!=''||$p_id!=''||$c_id!=''): ?><a href="<?php echo U('Ad/adlist');?>">取消查询</a><?php endif; ?>
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加广告</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        <tr class="center_t_t">
          ...
          <td width="135">广告名称</td>
          <td width="135">添加时间</td>
          <td width="135">位置</td>
          <td width="135">添加人</td>
          <td width="135">所属城市</td>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["addtimef"]); ?></td>
           <td width="135"><?php echo ($vo["weizhiname"]); ?></td>
          <td width="135"><?php echo ($vo["a_name"]); ?></td>
          <td width="135"><?php echo ($vo["p_name"]); ?>/<?php echo ($vo["c_name"]); ?></td>
          <td width="135">
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Ad/editad',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo ($systemConfig["SITE_INFO"]["url"]); echo ($vo["img"]); ?>" target="_blank" style="cursor:pointer">查看广告图片</a>
          </td>
