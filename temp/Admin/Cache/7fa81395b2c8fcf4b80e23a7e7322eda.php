...
<html>
<head>
<title>咨询列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：咨询列表</div>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["lxfs"]); ?></td>
          <td width="100"><?php echo ($vo["addtime_f"]); ?></td>
          <td width="100"><?php echo ($vo["p_name"]); ?></td>
          <td width="100"><?php echo ($vo["c_name"]); ?></td>
          <td width="100"><?php echo ($vo["status_f"]); ?></td>
          <td width="135">
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Zixun/edit_status',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
              
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Zixun/del_zixun',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
