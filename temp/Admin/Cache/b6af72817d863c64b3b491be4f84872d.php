...
<html>
<head>
<title>文章评论-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
    ...
    <div class="center">
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <select id="uid" name="uid" class="choose">
            ...
            <?php if(!empty($is_hf)||!empty($uid)): ?><a href="<?php echo U('Article/comments');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135">
              <a link="<?php echo U('Article/status_rcomments',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
        <?php if($vo[ishf]==0): ?><a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Article/hf_rcomments',array('id'=>$vo[id]));?>" style="cursor:pointer">回复</a><?php endif; ?>
              <a link="<?php echo U('Article/del_rcomments',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
