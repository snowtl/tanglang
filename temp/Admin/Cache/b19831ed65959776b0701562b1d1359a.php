...
<html>
<head>
<title>施工动态-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：施工动态</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <label>施工阶段</label>
        <select class="choose" id="jiduan" name="jieduan">
            <option value="">请选择</option>
            <?php if(is_array($jd)): $k = 0; $__LIST__ = $jd;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><option value="<?php echo ($k); ?>" <?php if($jieduan==$k): ?>selected<?php endif; ?> ><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
        <?php if($is_qx==0): if(!empty($p_id)||!empty($c_id)||!empty($keys)||!empty($uid)||!empty($jieduan)): ?><a href="<?php echo U('Gongzhang/index');?>">取消查询</a><?php endif; endif; ?>
        <?php if($is_qx==1): if(!empty($keys)||!empty($uid)||!empty($jieduan)): ?><a href="<?php echo U('Gongzhang/index');?>">取消查询</a><?php endif; endif; ?>
        
        <input name="sub" type="submit"  value="查询" class="sub1" />
        
        
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加施工动态</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <td width="135" alt='<?php echo ($vo["id"]); ?>' id='s_<?php echo ($vo["id"]); ?>' ondblclick="updatesort('<?php echo ($vo["id"]); ?>','<?php echo ($vo["sort"]); ?>')" ><?php echo ($vo["sort"]); ?></td>
          <td width="135"><?php echo ($vo["jieduans"]); ?></td>
          <td width="135"><?php echo ($vo["status_f"]); ?></td>
          
          <td width="135">
              <a name="<?php echo ($vo["truename"]); ?>" link="<?php echo U('Gongzhang/sgdtstatus',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Gongzhang/editsgdt',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
              <a name="<?php echo ($vo["truename"]); ?>" link="<?php echo U('Gongzhang/delsgtd',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
