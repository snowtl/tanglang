...
<html>
<head>
<title>参加活动记录-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：参加活动记录</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <label>团购活动</label>
        <select id="gid" name="gid" class="choose">
        <option value="">请选择</option>
        <?php if(is_array($glist)): $i = 0; $__LIST__ = $glist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($gid==$vo[id]): ?>selected<?php endif; ?>><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
       
        
        <input name="sub" type="submit"  value="查询" class="sub1" />
    
        <?php if($keys!=''||$g_id!=''): ?><a href="<?php echo U('Group/groupaddjl');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["p_name"]); ?>/<?php echo ($vo["c_name"]); ?>/<?php echo ($vo["q_name"]); ?>/<?php echo ($vo["xq_name"]); ?></td>
          <td width="135"><?php echo ($vo["statusf"]); ?></td>
          
          <td width="135">
              <a aid="<?php echo ($vo["name"]); ?>" class="xgzt" link="<?php echo U('Group/edit_jl_status',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer">修改状态</a>
                
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Group/del_jl',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
