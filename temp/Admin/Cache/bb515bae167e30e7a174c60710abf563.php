...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <head>
        <title>添加/编辑-客户-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
        <meta charset="UTF-8">
...
.preview {
 ...
 <div class="tabBox" id="Threetab">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：<a href="<?php echo U('Member/index');?>" class="a1">客户</a>>>>编辑/添加 客户信息</div>
      <div class="center_t">
          <form action="" id="form1" name="form1" method="post">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab center_t">
                ...
                <tr>
                    <td>姓名：</td>
                    <td><input type="text" name="dinfo[truename]" size="40" id="truename" value="<?php echo ($info["truename"]); ?>" /></td>
                </tr>
                <tr>
                    <td>头像上传：</td>
                    <td>
	...
	<div>
		<span style="vertical-align: top">裁切图片：</span>
                ...
                <tr>
                    <td>性别：</td>
                    <td>
                        <label><input type="radio" name="dinfo[sex]" id="sex" value="1" <?php if($info[sex]==1): ?>checked<?php endif; ?> />男</label>
                        <label><input type="radio" name="dinfo[sex]" id="sex" value="2" <?php if($info[sex]==2): ?>checked<?php endif; ?> />女</label>
                ...
                <tr>
                    <td>email：</td>
                    <td><input type="text" name="dinfo[email]" size="40" id="email" value="<?php echo ($info[email]); ?>" /></td>
                </tr>
                
                <tr>
                    <td>qq：</td>
                    <td><input type="text" name="dinfo[qq]" size="40" id="qq" value="<?php echo ($info[qq]); ?>" /></td>
                </tr>  
                <tr>
                    <td>
                    ...
                    <td><input type="text" name="dinfo[movphone]" size="40" id="movphone" value="<?php echo ($info[movphone]); ?>" /></td>
                </tr>
                <tr>
                    <td>生日：</td>
                    <td>
                    <input type="text" id="but" name="dinfo[birthday]" readonly value="<?php echo ($info[birthday]); ?>" />
                    </td>
                </tr>  
                <?php if($is_qx==0): ?><tr>
                    <td>省份/城市：</td>
                    <td>
                        <select id="province_id" name="dinfo[province_id]" onchange="getcity('province_id','city_id')" >
                        ...
                        <select id="city_id" name="dinfo[city_id]" onchange="getdiqu('province_id','city_id','qu_id')">
                            <option value="">请选择市</option>
                            <?php if(!empty($info[c_id])): ?><option value="<?php echo ($info["c_id"]); ?>" selected="selected"><?php echo ($info["city_name"]); ?></option><?php endif; ?>
                        </select>
                        
                        <select id="qu_id" name="dinfo[qu_id]">
                            <option value="">请选择区</option>
                            <?php if($info[q_id]): ?><option value="<?php echo ($info["q_id"]); ?>" selected="selected"><?php echo ($info["qu_name"]); ?></option><?php endif; ?>
                        </select>
                    ...
                    <td>地区：</td>
                    <td>
                        <input type="hidden" id="province_id" name="dinfo[province_id]" value="<?php echo ($proid); ?>" />
                        <input type="hidden" id="city_id" name="dinfo[city_id]" value="<?php echo ($cityid); ?>" />
                        <select id="qu_id" name="dinfo[qu_id]">
                            <option value="">请选择区</option>
                ...
                <tr>
                    <td>地址：</td>
                    <td><input type="text" name="dinfo[address]" size="40" id="address" value="<?php echo ($info[address]); ?>" /></td>
                </tr>
                <tr>
                    <td>积分：</td>
                    <td><input type="text" name="dinfo[jifen]" size="10" id="jifen" value="<?php echo ($info[jifen]); ?>" /></td>
                </tr>
                <tr>
                    <td>座机：</td>
                    <td><input type="text" name="dinfo[telphone]" size="40" id="telphone" value="<?php echo ($info[telphone]); ?>" /></td>
                </tr>
                <tr>
                    <td>户型：</td>
                    <td>
                        <select id="huxing" name="dinfo[huxing]">
                            <option value="">请选择户型</option>
                            <?php if(is_array($hxlist)): $i = 0; $__LIST__ = $hxlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($info[huxing]==$vo[id]): ?>selected<?php endif; ?> ><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                    </td>
                </tr>
                <tr>
                    <td>面积：</td>
                    <td><input type="text" name="dinfo[mianji]" size="40" id="mianji" value="<?php echo ($info[mianji]); ?>" /></td>
                </tr>
                <tr>
                    <td>装修阶段：</td>
                    <td>
                        <select id="zxjd" name="dinfo[zxjd]">
                            <option value="">请选择装修阶段</option>
                            <?php if(is_array($zxjdlist)): $k = 0; $__LIST__ = $zxjdlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><option value="<?php echo ($k); ?>" <?php if($info[zxjd]==$k): ?>selected<?php endif; ?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                    </td>
                </tr>
                <tr>
                    <td>喜欢风格：</td>
                    <td>
                        <select id="zxjd" name="dinfo[fengge]">
                            <option value="">请选择风格</option>
                            <?php if(is_array($fglist)): $i = 0; $__LIST__ = $fglist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($info[fengge]==$vo[id]): ?>selected<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                    </td>
                </tr>
                <tr>
                    <td>预算：</td>
                    <td>
                        <select id="yusuan" name="dinfo[yusuan]">
                            <option value="">请选择预算</option>
                            <?php if(is_array($yslist)): $i = 0; $__LIST__ = $yslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["ysid"]); ?>" <?php if($info[yusuan]==$vo[ysid]): ?>selected<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                    </td>
                </tr>
                <tr>
                    <td>装修时间：</td>
                    <td>
                        <input type="text" id="zxtime" name="dinfo[zxtime]" readonly value="<?php echo ($info[zxtime]); ?>" />
                    </td>
                </tr>
