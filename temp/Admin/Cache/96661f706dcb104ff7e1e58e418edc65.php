...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <head>
        <title>添加/编辑-明星工长-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
        <meta charset="UTF-8">
...
<script type="text/javascript" src="__PUBLIC__/datepicker/js/jquery-ui-timepicker-addon.js"></script>
 ...
 <div class="tabBox" id="Threetab">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：<a href="<?php echo U('Member/foreman');?>" class="a1">明星工长</a>>>>编辑/添加 明星工长信息</div>
      <div class="center_t">
	...
	<div class="preview-area">
		...
		<input type="hidden" id='img_src' name='src'/>
                <input type="hidden" id='picName' value="<?php echo ($info["f_logo"]); ?>" name='picName'/>
	...
	<div>
		<span style="vertical-align: top">裁切图片：</span>
                ...
                <tr>
                    <td>入职日期：</td>
                    <td>
                       <input type="text" name="ruzhitime" size="40" id="ruzhitime" value="<?php echo ($info["ruzhitime"]); ?>" />
                    </td>
                </tr>
                <tr>
                    <td>装修公司排名：</td>
                    <td>
                       <input type="text" name="paiming" size="10" id="paiming" value="<?php echo ($info["paiming"]); ?>" />
                    </td>
                </tr>
                ...
                <tr>
                    <td>公司名称：</td>
                    <td>
                       <input type="text" name="company" size="100" id="company" value="<?php echo ($info["company"]); ?>" />
                    </td>
                </tr>
                <tr>
                    <td>口碑值：</td>
                ...
                <tr>
                    <td>评论数：</td>
                    <td>
                       <input type="text" name="comments" size="10" id="comments" value="<?php echo ($info["comments"]); ?>" />
                    </td>
                </tr>
                <tr>
                    <td>施工团队数目：</td>
                    <td>
                       <input type="text" name="peoples" size="10" id="peoples" value="<?php echo ($info["peoples"]); ?>" />
                    </td>
                </tr>
                <tr>
                    <td>案例数目：</td>
                    <td>
                       <input type="text" name="cases" size="10" id="cases" value="<?php echo ($info["cases"]); ?>" />
                    </td>
                </tr>
                <tr>
                    <td>预约次数：</td>
                    <td>
                       <input type="text" name="yuyues" size="10" id="yuyues" value="<?php echo ($info["yuyues"]); ?>" />
                    </td>
                </tr>
                <tr>
                    <td>服务业主人数：</td>
                    <td>
                       <input type="text" name="fwyzrs" size="10" id="fwyzrs" value="<?php echo ($info["fwyzrs"]); ?>" />
                    </td>
                </tr>
                <tr>
                    <td>服务范围：</td>
                    <td>
                       ...
                       <textarea name="fuwu" cols="100" rows="10"><?php echo ($fuwuarr); ?></textarea>
                       <br/>
                       <?php if(empty($info[fuwu])): ?>备注：多种服务按enter键<?php endif; ?>
                ...
                <?php if($addgz!=1): ?><tr>
                    <td>代表作品：</td>
                    <td>
                        <select name="zuopin[]" id="zuopin" size="10" multiple="multiple">
                            <?php if(is_array($caselist)): $i = 0; $__LIST__ = $caselist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(in_array($vo[id],$zparr)): ?>selected<?php endif; ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                        备注按着Ctrl键的同时点击鼠标的左键进行多选
                    </td>
                </tr><?php endif; ?>
                <tr>
                    <td>是否进行身份证认证：</td>
                    <td>
                        ...
                        <label><input type="radio" id="is_sfz" name="is_sfz" value="1" <?php if($info[is_sfz]==1): ?>checked<?php endif; ?> /> 是</label>
                        <label><input type="radio" id="is_sfz" name="is_sfz" value="0" <?php if($info[is_sfz]==0): ?>checked<?php endif; ?> /> 否</label>
                    </td>
                </tr>
                <tr>
                    <td>是否签约：</td>
                    <td>
                        <label><input type="radio" id="is_qy" name="is_qy" value="1" <?php if($info[is_qy]==1): ?>checked<?php endif; ?> /> 是</label>
                        <label><input type="radio" id="is_qy" name="is_qy" value="0" <?php if($info[is_qy]==0): ?>checked<?php endif; ?> /> 否</label>
                    </td>
                </tr>
                <tr>
                    <td>签约时间：</td>
                    <td>
                        <input type="text" name="qytime" id="qytime" value="<?php echo ($info[qytime]); ?>" />
                    </td>
                </tr>
                <tr>
                   <td>是否缴纳保证金：</td>
                    <td>
                        <label><input type="radio" id="is_bzj" name="is_bzj" value="1" <?php if($info[is_bzj]==1): ?>checked<?php endif; ?> /> 是</label>
                        <label><input type="radio" id="is_bzj" name="is_bzj" value="0" <?php if($info[is_bzj]==0): ?>checked<?php endif; ?> /> 否</label>
                    </td>
                </tr>
                <tr>
                    <td>身份证图片</td>
                    <td>
                        <input type="file" name="sfz_img" id="sfz_img" value="" /><br/>
                        <?php if(!empty($info[sfz_img])): ?><img src="<?php echo ($systemConfig["SITE_INFO"]["url"]); echo ($info["sfz_img"]); ?>" width="100"/><?php endif; ?>
                    </td>
                </tr>
                <tr>
                    <td>个人简介</td>
                ...
                <tr>
                    <td>座机：</td>
                    <td><input type="text" name="telphone" id="telphone" value="<?php echo ($info["telphone"]); ?>" /></td>
                </tr>
                <tr>
                    <td>手机号：</td>
                    <td><input type="text" name="movphone" id="movphone" value="<?php echo ($info["movphone"]); ?>" /></td>
                </tr>
                ...
                <tr>
                    <td>email：</td>
                    <td><input type="text" name="email" size="40" id="email" value="<?php echo ($info[email]); ?>" /></td>
                </tr>                
                <tr>
                    <td>工龄：</td>
                    <td><input type="text" name="gongling" size="10" id="gongling" value="<?php echo ($info[gongling]); ?>" /></td>
                </tr>                
                <tr>
                    <td>是否推荐：</td>
                    <td>
                 ...
                 <tr>
                    <td>导航图片</td>
                    <td>
                        <input type="file" name="dhimg" id="dhimg" value="" /><br/>
                ...
                <tr>
                    <td>qq：</td>
                    <td><input type="text" name="qq" size="40" id="qq" value="<?php echo ($info[qq]); ?>" /></td>
                </tr>  
                ...
                <?php if($is_qx==0): ?><tr>
                    ...
                    <td>
                        ...
                        <select id="c_id" name="c_id" onchange="getdiqu('p_id','c_id','q_id')">
                            ...
                            <?php if(!empty($info[c_id])): ?><option value="<?php echo ($info["c_id"]); ?>" selected="selected"><?php echo ($info["cname"]); ?></option><?php endif; ?>
                        </select>
                        <select id="q_id" name="q_id">
                            <option value="">请选择市</option>
                            <?php if(!empty($info[q_id])): ?><option value="<?php echo ($info["q_id"]); ?>" selected="selected"><?php echo ($info["qname"]); ?></option><?php endif; ?>
                        </select>
                ...
                <?php if($is_qx==1): ?><tr>
                    ...
                    <td>
                        ...
                        <select id="q_id" name="q_id">
                            <option value="">请选择市</option>
                            <?php if(is_array($qulist)): $i = 0; $__LIST__ = $qulist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["region_id"]); ?>" <?php if($vo[region_id]==$info[q_id]): ?>selected<?php endif; ?> ><?php echo ($vo["region_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>  
...
$("#qytime").datetimepicker();
$("#ruzhitime").datetimepicker();
//地址
