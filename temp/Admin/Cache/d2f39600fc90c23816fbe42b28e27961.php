...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Shop/addshopcategory');?>";
	})
    ...
    <div class="center">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          <td class="td1" width="135"><?php echo ($vo["cid"]); ?></td>
          <td width="135"><?php echo ($vo["fullname"]); ?></td>
          <td width="135">
        <?php if(!empty($vo[clogo])): ?><img src="<?php echo ($systemConfig["SITE_INFO"]["url"]); echo ($vo["clogo"]); ?>"/><?php endif; ?>
        </td>
          
          <td width="135">
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Shop/editshopcategory',array('cid'=>$vo[cid]));?>" style="cursor:pointer">编辑</a>
              <a name="<?php echo ($vo["truename"]); ?>" link="<?php echo U('Shop/delshopcategory',array('cid'=>$vo[cid]));?>" style="cursor:pointer" class="del">删除</a></td>
