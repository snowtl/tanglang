...
<html>
<head>
<title>实体店列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Shiti/addst');?>";
	})
  ...
  <div class="center_r">
    <div class="center">
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if($is_qx==1): if($keys!=''||$q_id!=''): ?><a href="<?php echo U('Shiti/index');?>">取消查询</a><?php endif; ?>
            <?php else: ?>
        <?php if($keys!=''||$p_id!=''||$c_id!=''||$q_id!=''): ?><a href="<?php echo U('Shiti/index');?>">取消查询</a><?php endif; endif; ?>
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加实体店</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["telphone"]); ?></td>
          <td width="135"><?php echo ($vo["p_name"]); ?>/<?php echo ($vo["c_name"]); ?>/<?php echo ($vo["q_name"]); ?></td>
          <td width="135"><?php echo ($vo["isdis"]); ?></td>
          <td width="135">
              <a aid="<?php echo ($vo["id"]); ?>" class="xgzt" link="<?php echo U('Shiti/upstate',array('id'=>$vo[id],'status'=>$vo[is_display]));?>" style="cursor:pointer">修改状态</a>
              <a name="<?php echo ($vo["id"]); ?>" link="<?php echo U('Shiti/del',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          <a aid="<?php echo ($vo["id"]); ?>" href="<?php echo U('Shiti/editst',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
          </td>
