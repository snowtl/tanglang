...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <head>
        <title>编辑装修案例-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
        <meta charset="UTF-8">
 ...
 <div class="tabBox" id="Threetab">
  ...
  <div class="center_r">
    <div class="center">
      ...
      您的位置：<a href="<?php echo U('Gongzhang/list_case');?>" class="a1">装修案例</a>>>编辑装修案例
      </div>
      <div class="center_t">
          <form action="" method="POST" id="form1" name="form1" enctype="multipart/form-data" >
            ...
            <tr align="left"  class="center_t_d">
                <td width="20%">是否是经典</td>
                <td align="left">
                    <label><input type="radio" id="is_jd" name="is_jd" value="1" <?php if($info[is_jd]==1): ?>checked<?php endif; ?> />经典</label>
                    <label><input type="radio" id="is_jd" name="is_jd" value="0" <?php if($info[is_jd]==0): ?>checked<?php endif; ?> />普通</label>
                </td>
            </tr>
             <tr align="left"  class="center_t_d">
                <td width="20%">户型</td>
                <td align="left">
                    <select name="hid" id="hid">
                        <option value="">请选择</option>
                        <?php if(is_array($hxcategory)): $i = 0; $__LIST__ = $hxcategory;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo[id]==$info[hid]): ?>selected<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                   </select>
                </td>
            </tr>   
             <tr align="left"  class="center_t_d">
                ...
                <td align="left">
                    <select name="fid" id="fid">
                        <option value="">请选择</option>
                        <?php if(is_array($fgcategory)): $i = 0; $__LIST__ = $fgcategory;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo[id]==$info[fid]): ?>selected<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                   </select>
                </td>
            </tr>      
            ...
            <tr align="left"  class="center_t_d">
                <td width="20%">面积</td>
                <td align="left">
                    <input type="text" size="10" name="mianji" id="mianji" value="<?php echo ($info["mianji"]); ?>" />平米
                </td>
            </tr>    
            <tr align="left"  class="center_t_d">
                <td width="20%">价格</td>
                <td align="left">
                    <input type="text" size="10" name="price" id="price" value="<?php echo ($info["price"]); ?>" />元
                </td>
            </tr>  
            <tr align="left"  class="center_t_d">
                <td width="20%">好评数</td>
                <td align="left">
                    <input type="text" size="10" name="goods" id="goods" value="<?php echo ($info["goods"]); ?>" />
                </td>
            </tr> 
            <tr align="left"  class="center_t_d">
                <td width="20%">评价数</td>
                <td align="left">
                    <input type="text" size="10" name="comments" id="comments" value="<?php echo ($info["comments"]); ?>" />
                </td>
            </tr> 
            ...
            <?php if(!empty($info[fmimg])): ?><img src="__ROOT__<?php echo ($info["fmimg"]); ?>" height="100" /><?php endif; ?>
                ...
                <?php if(is_array($tpjhlist)): $k = 0; $__LIST__ = $tpjhlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr align="left"  class="center_t_d">
                  <td width="20%"><?php echo ($vo["name"]); ?>--图片集合 <a href="javascript:addimgrow('<?php echo ($vo["id"]); ?>');" class="btn submit">添加</a> </td>
                <td align="left">
                    <div id="tpjh_<?php echo ($vo["id"]); ?>">
                        <div class="rowtpjh">
                        <input type="file" name="imgsrc_<?php echo ($vo["id"]); ?>[]" id="imgsrc[]" value="" />
                        </div>
                    </div>
                    <?php if(is_array($imglist[$k][img])): $i = 0; $__LIST__ = $imglist[$k][img];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i;?><img src="__ROOT__<?php echo ($vo1); ?>" width="100"/><br/><?php endforeach; endif; else: echo "" ;endif; ?>
                ...
                <td width="20%">简介</td>
                <td align="left">
                    <textarea id="jianjie" name="jianjie"><?php echo ($info["jianjie"]); ?></textarea>
...
<script>
     var k=1;
        function addimgrow(id){
            
            var str="";
            str+='<div  class="rowtpjh">';
            str+='            <input type="file" name="imgsrc_'+id+'[]" id="imgsrc[]" value="" />';
            str+='            </div>';
            $("#tpjh_"+id).append(str);
            k++;
        }
