...
<html>
<head>
<title>文章分类-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
    ...
    <div class="center">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135" ondblclick="xg('<?php echo ($vo["cid"]); ?>','<?php echo ($vo["cname"]); ?>')" id="x_<?php echo ($vo["cid"]); ?>"><?php echo ($vo["cname"]); ?></td>
          <td width="135">
              
              <a name="<?php echo ($vo["cname"]); ?>" link="<?php echo U('Article/del_cat',array('cid'=>$vo[cid]));?>" style="cursor:pointer" class="del">删除</a></td>
...
function gbxg(id){
    ...
    $.ajax({
        ...
        datatype:"JSON",
        type:"POST",
        url:"<?php echo U('Article/xgcname');?>",
