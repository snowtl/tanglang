...
<html>
<head>
<title>参与"社区团装"参与记录-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：参与"社区团装"参与记录</div>
      <div class="center_r_z">
      <form method="get" name="search_form" id="search_form" action=""  >
        
          <label>社区团装列表</label>
          <input type="text" class="input1" onchange="searchsqtz(this.value)" name="keys" id="keys" value="" placeholder="输入名称搜索社区团装" />
              
          <select id="sqtzid" class="input1" name="sqtzid">
              <option value="">请选择</option>
              <?php if(is_array($sqtzlist)): $i = 0; $__LIST__ = $sqtzlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
          </select>
        <input name="sub" type="submit"  value="查询" class="sub1" />
        
        <?php if($sqtzid!=''): ?><a href="<?php echo U('Sqtz/list_tzjl');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["islx"]); ?></td>
          
          <td width="135">
              <a aid="<?php echo ($vo["id"]); ?>" class="xgzt" link="<?php echo U('Sqtz/update_status_tzjl',array('id'=>$vo[id],'status'=>$vo[is_lx]));?>" style="cursor:pointer">修改状态</a>
              <a name="<?php echo ($vo["id"]); ?>" link="<?php echo U('Sqtz/del_tzjl',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
...
<script>
    //搜索社区团装
    function searchsqtz(sqtz){
        $.ajax({
            data:"keys="+sqtz,
            cache:false,
            url:"<?php echo U('Sqtz/search_sqtz');?>",
            type:"POST",
            dataType:"Json",
            success:function(msg){
                ...
                var l;
                for(l=0;l<len;l++){
                    str+='<option value="'+msg[l].id+'">'+msg[l].name+'</option>';
