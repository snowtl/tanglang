...
<html>
<head>
<title>施工动态预约-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：施工动态预约</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <select name="sg_id" id="sg_id" class="choose" >
          <option value="" >请选择施工动态</option>
            <?php if(is_array($sglist)): $i = 0; $__LIST__ = $sglist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo[id]==$sg_id): ?>selected<?php endif; ?>><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
       
        
        <?php if(!empty($keys)||!empty($sg_id)): ?><a href="<?php echo U('Yuyue/yuyuelfjl');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135">
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Yuyue/edit_jl_status',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
              
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Yuyue/del_jl_status',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
