...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：口碑点评</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <select id="uid" name="uid" class="choose">
            ...
            <?php if(!empty($keys)||!empty($uid)): ?><a href="<?php echo U('Shop/koubeicomment');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["fwnum"]); ?></td>
          <td width="135"><?php echo ($vo["pznum"]); ?></td>
          <td width="135"><?php echo ($vo["xjnum"]); ?></td>
          <td width="100"><?php echo ($vo["is_good"]); ?></td>
          <td width="100"><?php echo ($vo["is_hf_f"]); ?></td>
          <td width="100"><?php echo ($vo["status_f"]); ?></td>
          <td width="135">
              <a link="<?php echo U('Shop/status_koubei',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
        <?php if($vo[is_hf]==0): ?><a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Shop/hf',array('id'=>$vo[id]));?>" style="cursor:pointer">回复</a><?php endif; ?>
              <a link="<?php echo U('Shop/del_koubei',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
