...
<html>
<head>
<title>图库列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Tuku/add_tuku');?>";
	})
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：图库列表</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <select id="uid" name="uid" class="choose">
            ...
            <?php if(!empty($keys)||!empty($uid)): ?><a href="<?php echo U('Tuku/index');?>">取消查询</a><?php endif; ?>
        
        
        <input name="sub" type="submit"  value="查询" class="sub1" />
        
        
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加图库</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <td width="135"><?php echo ($vo["shoucang"]); ?></td>
          <td width="135"><?php echo ($vo["click"]); ?></td>
          <td width="135"><?php echo ($vo["status_f"]); ?></td>
          
          <td width="135">
              <a name="<?php echo ($vo["title"]); ?>" link="<?php echo U('Tuku/edit_status',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Tuku/edit_tuku',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
              <a name="<?php echo ($vo["title"]); ?>" link="<?php echo U('Tuku/del_tuku',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
