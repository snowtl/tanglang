...
<html>
<head>
<title>设计师设计-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
  ...
  <div class="center_r">
    <div class="center">
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if(!empty($keys)||!empty($p_id)||!empty($c_id)): ?><a href="<?php echo U('Yuyue/gzyy');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135">
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Yuyue/edit_status',array('id'=>$vo[id],'status'=>$vo[status],'is_gz'=>0));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
              
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Yuyue/del_yuyue',array('id'=>$vo[id],'is_gz'=>0));?>" style="cursor:pointer" class="del">删除</a>
          </td>
