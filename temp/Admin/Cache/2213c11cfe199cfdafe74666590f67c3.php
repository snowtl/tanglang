...
<html>
<head>
<title>预算-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Yusuan/addys');?>";
	})
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：预算</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if($keys!=''): ?><a href="<?php echo U('Yusuan/index');?>">取消查询</a><?php endif; ?>
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加预算</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          <td class="td1" width="135"><?php echo ($vo["ysid"]); ?></td>
          <td width="135"><?php echo ($vo["name"]); ?></td>
          <td width="135">
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Yusuan/editys',array('id'=>$vo[ysid]));?>" style="cursor:pointer">编辑</a>
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Yusuan/delys',array('id'=>$vo[ysid]));?>" style="cursor:pointer" class="del">删除</a></td>
