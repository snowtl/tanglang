...
<html>
<head>
<title>好友关系列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：好友关系列表</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        的好友列表
            <?php if(!empty($uid)): ?><a href="<?php echo U('Gongzhang/firendgx');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["a_name"]); ?>[<?php echo ($vo["truename"]); ?>]</td>
          <td width="135"><?php echo ($vo["a_name1"]); ?>[<?php echo ($vo["truename1"]); ?>]</td>
          <td width="135">             
              <a link="<?php echo U('Gongzhang/del_flink',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
