...
<html>
<head>
<title>添加优惠券门店-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<style>
    .input1{ border: 1px solid #005DAD; width: 500px;}
</style>
<body>
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
        <div class="center_r_t">您的位置：<a class="a1" href="<?php echo U('Shop/list_yhq');?>">优惠券列表</a>>>
            <?php if($is_edit==1): ?><a href="<?php echo U('Shop/ck_mendian',array('id'=>$info[yhqid]));?>" class="a1">优惠券门店列表</a>
                <?php else: ?>
                <a href="<?php echo U('Shop/ck_mendian',array('id'=>$id));?>" class="a1">优惠券门店列表</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        <tr class="center_t_d">
          <td class="td1" width="135">优惠券门店名称</td>
          <td><input type="text" class="input1" name='mdname' id="mdname" value='<?php echo ($info["mdname"]); ?>' />
          </td>
        </tr>
        <tr class="center_t_d">
          <td colspan='2'>
        ...
        <?php if($is_edit==1): ?><input type='hidden' id="yhqid" name='yhqid' value='<?php echo ($info[yhqid]); ?>' /><?php endif; ?>
              <input type="submit" class='btn submit' name='sub' id="sub" value='提交' />
          </td>
        </tr>
      </table>
      </form>
