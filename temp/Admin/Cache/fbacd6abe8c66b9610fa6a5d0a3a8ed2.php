...
<html>
<head>
<title>视频列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Shipin/add_shipin');?>";
	})
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：视频列表</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <label>会员</label>
        <select id="type" name="type" class="choose" onchange="gbmember(this.value)">
        ...
        <select id="c_id" name="c_id" class="choose">
            ...
            <?php if(!empty($keys)||!empty($uid)||!empty($p_id)||!empty($c_id)): ?><a href="<?php echo U('Shipin/index');?>">取消查询</a><?php endif; ?>
        
        
        <input name="sub" type="submit"  value="查询" class="sub1" />
        
        
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加视频</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["click"]); ?></td>
          <td width="135"><?php echo ($vo["status_f"]); ?></td>
          
          <td width="135">
              <a name="<?php echo ($vo["title"]); ?>" link="<?php echo U('Shipin/edit_status',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Shipin/edit_shipin',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
              <a name="<?php echo ($vo["title"]); ?>" link="<?php echo U('Shipin/del_shipin',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
