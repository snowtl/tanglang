...
<html>
<head>
<title>实体店幻灯片列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Shiti/addsthdp');?>";
	})
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：实体店幻灯片列表</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if($is_qx==1): ?><input type="hidden" id="p_id" name="p_id" value="<?php echo ($p_id); ?>" />
        ...
        <select id="stid" name="stid">
            ...
            <?php if(is_array($stlist)): $i = 0; $__LIST__ = $stlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($stid==$vo[id]): ?>selected<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
         <input name="sub" type="submit"  value="查询" class="sub1" />
        <?php if($is_qx==1): if($keys!=''||$q_id!=''||$stid!=''): ?><a href="<?php echo U('Shiti/liststhdp');?>">取消查询</a><?php endif; ?>
        <?php else: ?>
        <?php if($keys!=''||$p_id!=''||$c_id!=''||$q_id!=''||$stid!=''): ?><a href="<?php echo U('Shiti/liststhdp');?>">取消查询</a><?php endif; endif; ?>
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加实体店幻灯片</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><a href="<?php echo ($vo["link"]); ?>" target="_blank"><img src="<?php echo ($systemConfig["SITE_INFO"]["url"]); echo ($vo["img"]); ?>" width="100" /></a></td>
          <td width="135"><?php echo ($vo["addtimef"]); ?></td> 
          <td width="135"><?php echo ($vo["title"]); ?></td>
          <td width="135"><?php echo ($vo["isdis"]); ?></td>
          <td width="135">
              <a aid="<?php echo ($vo["id"]); ?>" class="xgzt" link="<?php echo U('Shiti/upstatuehdp',array('id'=>$vo[id],'status'=>$vo[is_display]));?>" style="cursor:pointer">修改状态</a>
              <a name="<?php echo ($vo["id"]); ?>" link="<?php echo U('Shiti/delsthdp',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          <a aid="<?php echo ($vo["id"]); ?>" href="<?php echo U('Shiti/editsthdp',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
          </td>
