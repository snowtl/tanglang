...
<html>
<head>
<title>装修日记-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Gongzhang/add_riji');?>";
	})
})
function close_win(){
	$(".theme-popover").html("");
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：装修日记</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if($is_qx==0): if(!empty($p_id)||!empty($c_id)||!empty($keys)||!empty($uid)): ?><a href="<?php echo U('Gongzhang/list_riji');?>">取消查询</a><?php endif; endif; ?>
        <?php if($is_qx==1): if(!empty($keys)||!empty($uid)): ?><a href="<?php echo U('Gongzhang/list_riji');?>">取消查询</a><?php endif; endif; ?>
        
        <input name="sub" type="submit"  value="查询" class="sub1" />
        
        
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加装修日记</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <td width="135" alt='<?php echo ($vo["id"]); ?>' id='s_<?php echo ($vo["id"]); ?>' ondblclick="updatesort('<?php echo ($vo["id"]); ?>','<?php echo ($vo["click"]); ?>')" ><?php echo ($vo["click"]); ?></td>
          
          <td width="135"><?php echo ($vo["status_f"]); ?></td>
          
          <td width="135">
              <a name="<?php echo ($vo["title"]); ?>" link="<?php echo U('Gongzhang/rijistatus',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Gongzhang/edit_riji',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
              <a name="<?php echo ($vo["title"]); ?>" link="<?php echo U('Gongzhang/del_riji',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
...
<script>
    ...
    function gb(sort,id){
        $.ajax({
            ...
            url:"<?php echo U('Gongzhang/ajaxupdate_rj_click');?>",
            type:"POST",
            datatype:"JSON",
            cache:false,
