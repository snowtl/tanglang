...
<html>
<head>
<title>团购活动-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Group/add_group');?>";
	})
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：团购活动</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if($is_qx==0): ?><label>省</label>
        <select id="p_id" name="p_id" onchange="getcity('p_id','c_id')" >
                        <option value="">请选择省</option>
                        <?php if(is_array($plist)): $i = 0; $__LIST__ = $plist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["region_id"]); ?>" <?php if($p_id==$vo[region_id]): ?>selected<?php endif; ?>><?php echo ($vo["region_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
        <label>市</label>
        ...
        <label>区</label>
        <select id="q_id" name="q_id" ><option value="">请选择区</option>
            <?php if(!empty($q_id)): ?><option value="<?php echo ($q_id); ?>" selected=""><?php echo ($q_name); ?></option><?php endif; ?>
        </select><?php endif; ?>
        <?php if($is_qx==1): ?><label>区</label>
        <select id="q_id" name="q_id" ><option value="">请选择区</option>
            
            <?php if(is_array($qlist)): $i = 0; $__LIST__ = $qlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["region_id"]); ?>" <?php if($q_id==$vo[region_id]): ?>selected<?php endif; ?>><?php echo ($vo["region_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select><?php endif; ?>
        
        <input name="sub" type="submit"  value="查询" class="sub1" />
        <?php if($is_qx==1): if($keys!=''||$q_id!=''): ?><a href="<?php echo U('Group/index');?>">取消查询</a><?php endif; ?>
            <?php else: ?>
        <?php if($keys!=''||$p_id!=''||$c_id!=''||$q_id!=''): ?><a href="<?php echo U('Group/index');?>">取消查询</a><?php endif; endif; ?>
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加团购活动</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        <tr class="center_t_t">
          ...
          <td width="135">团装名称</td>
          <td width="135">添加时间</td>
        <td width="135">报名人数</td>
        <td width="135">城市</td>
          <td width="135">状态</td>
    <td width="135">是否推荐</td>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["title"]); ?></td>
          <td width="135"><?php echo ($vo["addtimef"]); ?></td>
          <td width="135"><?php echo ($vo["bmnum"]); ?></td>
          <td width="135"><?php echo ($vo["p_name"]); ?>/<?php echo ($vo["c_name"]); ?>/<?php echo ($vo["q_name"]); ?></td>
          <td width="135"><?php echo ($vo["statusf"]); ?></td>
          <td width="135"><?php echo ($vo["is_tjf"]); ?></td>
          <td width="135">
              <a aid="<?php echo ($vo["title"]); ?>" class="xgzt" link="<?php echo U('Group/edit_status',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer">修改状态</a>
              <a aid="<?php echo ($vo["title"]); ?>" class="xgzt" link="<?php echo U('Group/edit_tj',array('id'=>$vo[id],'is_tj'=>$vo[is_tj]));?>" style="cursor:pointer">修改推荐</a><Br/>
              <a name="<?php echo ($vo["title"]); ?>" link="<?php echo U('Group/del_group',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          <a aid="<?php echo ($vo["title"]); ?>" href="<?php echo U('Group/edit_group',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
          </td>
...
//删除
    ...
    //修改排序
    function updatepx(id,or){
        $("#t_"+id).html("<input type='text' size='5' class='or'  id='input_"+id+"' name='input_"+id+"' value='"+or+"' onchange='editorder("+id+")' />");
    ...
    function editorder(id){
        ...
        $.ajax({
            ...
            cache:false,
            url:"<?php echo U('Sqtz/update_orders_sqtz');?>",
            type:"POST",
            dataType:"Json",
