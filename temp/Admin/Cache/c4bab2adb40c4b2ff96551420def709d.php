...
<html>
<head>
<title>留言咨询-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：留言咨询</div>
      <div class="center_r_z">
      <form method="get" name="search_form" id="search_form" action=""  >
        <label>是否回复</label>
        <select id="is_hf" name="is_hf" class="choose">
            <option value="">请选择</option>
            <option value="2" <?php if($is_hf==2): ?>selected<?php endif; ?> >未回复</option>
            <option value="1" <?php if($is_hf==1): ?>selected<?php endif; ?> >已回复</option>
        </select>
        <label>留言类型</label>
        <select id="lytype" name="lytype" class="choose">
            <option value="">请选择</option>
            <?php if(is_array($messagetype)): $k = 0; $__LIST__ = $messagetype;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><option value="<?php echo ($k); ?>" <?php if($k==$lytype): ?>selected<?php endif; ?> ><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
        <label>商城</label>
        <select id="uid" name="uid" class="choose">
            <option value="">请选择</option>
            <?php if(is_array($gzlist)): $i = 0; $__LIST__ = $gzlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["a_id"]); ?>" <?php if($uid==$vo[a_id]): ?>selected<?php endif; ?> ><?php echo ($vo["a_name"]); ?>[<?php echo ($vo["lxrname"]); ?>][<?php echo ($vo["company"]); ?>]</option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
            <?php if(!empty($is_hf)||!empty($uid)||!empty($lytype)): ?><a href="<?php echo U('Shop/message');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        <tr class="center_t_t">
          ...
          <td width="135">留言内容</td>
          <td width="135">是否回复</td>
          <td width="100">状态</td>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["lycontent"]); ?></td>
          <td width="100"><?php echo ($vo["ishf_f"]); ?></td>
          <td width="100"><?php echo ($vo["status_f"]); ?></td>
          <td width="135">
              <a link="<?php echo U('Shop/status_message',array('id'=>$vo[id],'status'=>$vo[status]));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
        <?php if($vo[ishf]==0): ?><a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Shop/hf_message',array('id'=>$vo[id]));?>" style="cursor:pointer">回复</a><?php endif; ?>
              <a link="<?php echo U('Shop/del_message',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a>
          </td>
