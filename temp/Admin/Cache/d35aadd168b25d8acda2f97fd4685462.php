...
<html>
<head>
<title>意见列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Yijian/addfriendlink');?>";
	})
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：意见列表</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if($keys!=''||$p_id!=''||$c_id!=''): ?><a href="<?php echo U('Yijian/index');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><a href="<?php echo ($vo["link"]); ?>" target="_blank"><?php echo ($vo["content"]); ?></a></td>
          ...
          <td width="135"><?php echo ($vo["islxf"]); ?></td>
          <td width="135">
              <a aid="<?php echo ($vo["id"]); ?>" class="xgzt" link="<?php echo U('Yijian/upstatus',array('id'=>$vo[id],'status'=>$vo[is_lx]));?>" style="cursor:pointer">修改状态</a>
              <a name="<?php echo ($vo["id"]); ?>" link="<?php echo U('Yijian/del',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a></td>
