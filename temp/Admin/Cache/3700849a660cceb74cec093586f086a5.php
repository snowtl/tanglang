...
<html>
<head>
<title>工长预约-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：工长预约</div>
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if($is_qx==0): ?><select name="p_id" id="p_id" class="choose" onChange="getcity('p_id','c_id')">
          <option value="" >请选择省</option>
          
			<?php if(is_array($p_list)): $i = 0; $__LIST__ = $p_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["region_id"]); ?>" <?php if($vo[region_id]==$p_id): ?>selected<?php endif; ?>><?php echo ($vo["region_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        ...
        <?php if(!empty($keys)||!empty($p_id)||!empty($c_id)): ?><a href="<?php echo U('Yuyue/index');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["ytname"]); ?></td>
          <td width="100"><?php echo ($vo["quyu"]); ?></td>
          <td width="100"><?php echo ($vo["mianji"]); ?></td>
          <td width="100"><?php echo ($vo["yusuan"]); ?></td>
          <td width="100"><?php echo ($vo["yaoqiu"]); ?></td>
          <td width="100"><?php echo ($vo["status_f"]); ?></td>
          
          <td width="135">
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Yuyue/edit_status',array('id'=>$vo[id],'status'=>$vo[status],'is_gz'=>1));?>" style="cursor:pointer" class="sgdtstatus">修改状态</a><br/>
              
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Yuyue/del_yuyue',array('id'=>$vo[id],'is_gz'=>1));?>" style="cursor:pointer" class="del">删除</a>
          </td>
