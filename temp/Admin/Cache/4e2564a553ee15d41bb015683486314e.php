      ...
      <div class="center1">
            ...
            <?php if($is_qx==0): ?><select name="province" id="province_s" class="choose" onChange="getcity('province_s','city_s')">
              ...
              <select name="city" id="city_s" class="choose" onChange="getdiqu('province_s','city_s','q_s')">
               ...
               <select name="qu" id="q_s" class="choose">
                ...
                <?php if(!empty($qu_id)): ?><option value="<?php echo ($qu_id); ?>" selected="selected"><?php echo ($quname); ?></option><?php endif; ?>
              </select><?php endif; ?>
            <?php if($is_qx==1): ?><input type="hidden" name="province" id="province"  value="<?php echo ($province); ?>" />
                <input type="hidden" name="city" id="city"  value="<?php echo ($city); ?>" />
                <select name="qu" id="q_s" class="choose">
                <option value="" >请选择区</option>
                <?php if(is_array($qulist)): $i = 0; $__LIST__ = $qulist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["region_id"]); ?>" 
                  <?php if($vo[region_id]==$qu_id): ?>selected<?php endif; ?>
                  ><?php echo ($vo["region_name"]); ?>
                  </option><?php endforeach; endif; else: echo "" ;endif; ?>
                
              </select><?php endif; ?>
            <?php if($is_qx==1): if(!empty($qu_id)||!empty($keys)||!empty($start_date)||!empty($end_date)): ?><a href="<?php echo U('Member/dianpu');?>">取消查询</a><?php endif; endif; ?>
            <?php if($is_qx==0): if(!empty($province)||!empty($city)||!empty($qu_id)||!empty($keys)||!empty($start_date)||!empty($end_date)): ?><a href="<?php echo U('Member/dianpu');?>">取消查询</a><?php endif; endif; ?>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
          ...
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
              ...
              <td width="135"><?php echo ($vo["jifen"]); ?></td>
              <td width="100"><?php echo ($vo["orders"]); ?></td>
              
              <td width="135"> <a aid="<?php echo ($vo["a_id"]); ?>" class="edituser" href="<?php echo U('Member/edit_dianpu',array('aid'=>$vo[a_id]));?>" style="cursor:pointer">编辑</a> <a name="<?php echo ($vo["company"]); ?>" link="<?php echo U('Member/del_dianpu',array('aid'=>$vo[a_id]));?>" style="cursor:pointer" class="del">删除</a></td>
