...
<html>
<head>
<title>文章列表-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Article/add_art');?>";
	})
...
<div class="wrap"> <div class="header">
  ...
  <div class="center_r">
    <div class="center">
      <div class="center_r_t">您的位置：文章列表</div>
      <div class="center_r_z">
      <form method="get" name="search_form" id="search_form" action=""  >
        <label>关键字</label>
        <input type="text" class="input1" size="100" name="keys"  <?php if($keys==''): ?>value="请输入关键字" <?php else: ?> value="<?php echo ($keys); ?>"<?php endif; ?> />
        <input name="sub" type="submit"  value="查询" class="sub1" />
        <?php if(!empty($keys)): ?><a href="<?php echo U('Article/index');?>"> 取消查询</a><?php endif; ?>
        <a class="btn btn-primary btn-large theme-login" href="javascript:;">添加文章</a>
        </form>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["uname"]); ?></td>
          <td width="100"><?php echo ($vo["addtime_f"]); ?></td>
          <td width="135">
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Article/edit_status',array('id'=>$vo[id],'status'=>$vo['status']));?>" style="cursor:pointer">修改状态</a>
              <Br/>
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Article/edit_tj',array('id'=>$vo[id],'is_tj'=>$vo['is_tj']));?>" style="cursor:pointer">修改推荐状态</a>
              <Br/>
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Article/edit_art',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
              <a name="<?php echo ($vo["title"]); ?>" link="<?php echo U('Article/del_art',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a></td>
