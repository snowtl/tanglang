...
<html>
<head>
<title>友情链接-后台管理-<?php echo ($systemConfig["SITE_INFO"]["name_cms"]); ?></title>
<meta charset="UTF-8">
...
jQuery(document).ready(function($) {
	$('.theme-login').click(function(){
		window.location.href="<?php echo U('Firendlink/addfriendlink');?>";
	})
  ...
  <div class="center_r">
    <div class="center">
      ...
      <form method="get" name="search_form" id="search_form" action=""  >
        ...
        <?php if($keys!=''||$p_id!=''||$c_id!=''): ?><a href="<?php echo U('Firendlink/index');?>">取消查询</a><?php endif; ?>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="center_t" style="word-break:break-all; word-wrap:break-all;">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="center_t_d">
          ...
          <td width="135"><?php echo ($vo["addtimef"]); ?></td>
           <td width="135"><?php echo ($vo["orders"]); ?></td>
          <td width="135"><?php echo ($vo["adduname"]); ?></td>
          <td width="135"><?php echo ($vo["p_name"]); ?>/<?php echo ($vo["c_name"]); ?></td>
          <td width="135">
              <a aid="<?php echo ($vo["a_id"]); ?>" href="<?php echo U('Firendlink/editfirendlink',array('id'=>$vo[id]));?>" style="cursor:pointer">编辑</a>
              <a name="<?php echo ($vo["name"]); ?>" link="<?php echo U('Firendlink/del',array('id'=>$vo[id]));?>" style="cursor:pointer" class="del">删除</a></td>
